...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    ds=p
    ds[np.arange(p.shape[0]),y]-=1
    ds /= N

    dl2=ds.dot(W2.T)
    dw2=l2.T.dot(ds) +2*reg*W2
    db2= np.sum(ds, axis=0) #equivlaent to ones(1,N).dot

    dl1=dl2
    dl1[l1<0]=0 #same gradient for positive

    db1=np.sum(dl1, axis=0)
    dw1=X.T.dot(dl1) + 2*reg*W1
  ...
  def predict(self, X):
    ...
    s= l1.dot(self.params['W2']) +self.params['b2']
