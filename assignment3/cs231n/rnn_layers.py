...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h=h0#first hidden state
    x = x.transpose(1,0,2) #(T,N,D)
    h=np.zeros((T,N,H))
    cache=[]
    for t in range(T):
        h[t],c = rnn_step_forward(x[t],prev_h,Wx,Wh,b)
        cache.append(c)
        prev_h=h[t]
...
def rnn_backward(dh, cache):
    ...
    #n3ks elly 3amalnah from end to start
    for t in reversed(range(T)):
        dh_current = dh[t] + dh_prev  # 2 gradients flowing to each state one from dh and another from rnn_step_backward
